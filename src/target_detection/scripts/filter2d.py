import numpy as np
from collections import deque
from scipy.signal import butter, sosfilt, sosfilt_zi

# ---------------- 2 通道 5 点滑动中值 ----------------
class Median5_2D:
    def __init__(self):
        # 每条通道各自维护一个 deque
        self.buf = [deque(maxlen=5) for _ in range(2)]

    def update(self, x: np.ndarray) -> np.ndarray:
        """x: shape=(2,)  返回同 shape 的滤波结果"""
        out = np.empty(2)
        for i in range(2):
            self.buf[i].append(x[i])
            out[i] = np.median(self.buf[i])
        return out


# ---------------- 2 通道 Butterworth 低通 ----------------
class LowPassRealTime2D:
    def __init__(self, fs, fc):
        sos = butter(2, fc, btype='low', fs=fs, output='sos')
        self.sos = sos                                  
        self.sos_zi   = sosfilt_zi(sos)                     
        self.sos_zi   = np.tile(self.sos_zi, (2, 1, 1, 1))

    def update(self, x: np.ndarray) -> np.ndarray:
        """x: shape=(2,)  返回同 shape 的滤波结果"""
        y = np.empty(2)
        for i in range(2):
            y[i], self.sos_zi[i] = sosfilt(self.sos, [[x[i]]], zi=self.sos_zi[i])
        return y


# ---------------- 2 通道 “中值+低通” 级联 ----------------
class MedianThenLowPass2D:
    def __init__(self, fs, fc):
        self.median = Median5_2D()
        self.lpf    = LowPassRealTime2D(fs, fc)

    def update(self, xyz_raw: np.ndarray) -> np.ndarray:
        xyz_med = self.median.update(xyz_raw)
        xyz_lpf = self.lpf.update(xyz_med)
        return xyz_lpf


if __name__ == "__main__":
    fs, fc = 200, 5.0          # 采样频率fc，低通截止频率fc
    filter   = MedianThenLowPass2D(fs, fc)

    # 模拟一条实时数据流
    for k in range(1000):
        xyz_new = np.random.randn(2)
        xyz_flt = filter.update(xyz_new) 
        print(k, xyz_flt)
