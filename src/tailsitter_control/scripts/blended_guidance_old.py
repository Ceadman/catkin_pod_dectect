...
class Proportional_Navigation_Law:
    def __init__(self):
        ...
        self.omega_los_map_future = np.array([0, 0, 0])
        ...
        self.g = 9.81
        self.omega_pred = None
        

    def detection_distance_cb(self, msg: Float32):
    #     t = rospy.Time.now().to_sec()
        rospy.loginfo_throttle(1.0, f"distance:{msg.data:.2f}")
        
    #     if getattr(self, 'last_dist_t', None) is None:
    #         self.distance = dist_new
    #         self._det_Vc = 0.0
            
    #         self.last_dist_t = t
    #         self.last_dist = dist_new
    #         return
        
    #     dt = max(t - self.last_dist_t, 1e-6)
    #     self.distance = dist_new
        
    #     if dt > DT*1 and dist_new != self.last_dist:
    #         d_dist = dist_new - self.last_dist
    #         #rospy.logwarn(f"d_dist={d_dist:.4f}, dt={dt:.4f}, "f"new={dist_new:.2f}, old={self.last_dist:.2f}")
    #         self.det_Vc = -d_dist / dt  
            
    #         self.last_dist_t = t
    #         self.last_dist = dist_new
    #         rospy.logwarn_throttle(1.0, f"[PN Guidance] Vc: {self.det_Vc:.2f} m/s")
    ...
    def is_it_lost(self, msg: PointStamped):
        self.target_in_img = False
        margin_x = self.pod_width / 10
        margin_y = self.pod_height / 10
        x_min = margin_x
        x_max = self.pod_width - margin_x
        y_min = margin_y
        y_max = self.pod_height - margin_y
        px = msg.point.x
        py = msg.point.y
    ...
    def predict_omega_linear(self, omega_now, t_now, 
                      predict_horizon=1.0, max_accel_deg=5.0, window_size=10):
        """
        滑动窗口线性预测视线角速度（类内即插即用）
        
        基于最近N个数据点线性拟合外推，不依赖飞机动力学模型，
        适用于速度控制或参数未知的场景。
        
        Args:
            omega_now: 当前视线角速度 (rad/s)
            t_now: 当前时间戳 (s)，可用rospy.Time.now().to_sec()
            predict_horizon: 预测时间步长 (s)，默认1.0
            max_accel_deg: 物理限制，最大角加速度 (°/s^2)，默认5.0
            window_size: 滑动窗口大小，默认10
        
        Returns:
            omega_pred: 预测的未来视线角速度 (rad/s)
        """
        # 即插即用初始化（首次调用自动创建）
        if not hasattr(self, '_omega_buffer'):
            self._omega_buffer = deque(maxlen=window_size)
            self._max_omega_accel = np.radians(max_accel_deg)
        
        buffer = self._omega_buffer
        
        # 添加当前数据点
        buffer.append((t_now, omega_now))
        ...
        # 线性拟合计算斜率
        t_vec = np.array([x[0] for x in buffer])
        w_vec = np.array([x[1] for x in buffer])
        
        t_mean = t_vec.mean()
        t_center = t_vec - t_mean
        ...
        # 角加速度估计 (rad/s^2)
        omega_dot = np.sum(t_center * (w_vec - w_vec.mean())) / denom
        
        # 线性外推
        omega_pred = omega_now + omega_dot * predict_horizon
        
        # 物理限制：防止噪声发散
        max_delta = self._max_omega_accel * predict_horizon
        omega_pred = np.clip(omega_pred, omega_now - max_delta, omega_now + max_delta)
        
        return omega_pred

    def reset_omega_predictor(self):
        """重置滑动窗口历史数据（切换目标或重新初始化时调用）"""
        if hasattr(self, '_omega_buffer'):
    ...
    def pod_velocity_cb(self, msg: Vector3Stamped):
        ...
        t = msg.header.stamp.to_sec()
        if PREFORE_GUIDANCE:
            self.omega_los_map = np.array([msg.vector.x, msg.vector.y])
        else:
            self.omega_los_map_future = np.array([msg.vector.x, msg.vector.y])

        if PREFORE_GUIDANCE:
                # 协调转弯模型预测，准确、但只能预测水平
                commanded_accel_raw, force_break = pn_law.get_commanded_accel(pn_law.omega_los_map)
                q_raw, thrust_cmd_raw, roll_cmd_raw, pitch_cmd_raw = pn_law.vector_to_attitude(commanded_accel[0], commanded_accel[1], commanded_accel[2])
                self.omega_pred = pn_law.predict_omega_fixedwing(pn_law.omega_los_map, 
                                                                roll_cmd_raw, 
                                                                pitch_cmd_raw, 
                                                                np.array([pn_law.linear_velocity.x, pn_law.linear_velocity.y, pn_law.linear_velocity.z]), 
                                                                pre_time=1.0, 
                                                                T_roll=0.8)
                alpha = 0.2 + 0.6 * np.clip((pn_law.est_Vc - 10) / 30, 0, 1)
                # alpha = 1
                pn_law.omega_los_map_future = (1 - alpha) * pn_law.omega_los_map + alpha * self.omega_pred

                # 线性预测
                self.omega_pred = pn_law.predict_omega_linear(
                    omega_now = pn_law.omega_los_map,
                    t_now = rospy.Time.now().to_sec(), 
                    window_size = 10,      # 调节平滑度
                    predict_horizon = 1.0, # 预测1秒后的值
                    max_accel_deg = 5.0    # 根据飞机机动能力调整
                )
    ...
    def pn_guidance(self, omega_los_map):
        ...
        Nxy_eff = 2 # NXY * np.clip(gain, 0.2, 1.0)
        Nz_eff = 6 # NZ * np.clip(gain, 0.4, 1.0)
    ...
    def compute_target_velocity_3d(self):
        ...
        pod_angle_in_map[0]: 方位角（从东向北偏，逆时针为正）[rad]
        pod_angle_in_map[1]: 俯仰角（从水平面向上为正）[rad]
        """
        # 当前速度向量
        v_current = np.array([
            self.linear_velocity.x,
    ...
    def vector_to_attitude(self, ax_cmd, ay_cmd, az_cmd):
        """
        ...
        if L_mag < 0.1:
            # 指令过小，保持平飞
            return (tf.transformations.quaternion_from_euler(0, 0, self.euler_angles[2]), 0.5)
        ...
        # 实际上应该使用绝对航向
        q = tf.transformations.quaternion_from_euler(roll, pitch, self.euler_angles[2], axes='sxyz')
...
if __name__ == "__main__":
    ...
    rospy.Subscriber("/detection/ultralytics/target_point_pub", PointStamped, pn_law.is_it_lost)
    ...
    rospy.Subscriber("/detection/distance", Float32, pn_law.detection_distance_cb)
    ...
    while not rospy.is_shutdown():
            ...
            if PREFORE_GUIDANCE: 
                if pn_law.omega_pred == None:
                 # 协调转弯模型预测，准确、但只能预测水平
                    commanded_accel_raw, force_break = pn_law.get_commanded_accel(pn_law.omega_los_map)
                    q_raw, thrust_cmd_raw, roll_cmd_raw, pitch_cmd_raw = pn_law.vector_to_attitude(commanded_accel[0], commanded_accel[1], commanded_accel[2])
                    pn_law.omega_pred = pn_law.predict_omega_fixedwing(pn_law.omega_los_map, 
                                                                roll_cmd_raw, 
                                                                pitch_cmd_raw, 
                                                                np.array([pn_law.linear_velocity.x, pn_law.linear_velocity.y, pn_law.linear_velocity.z]), 
                                                                pre_time=1.0, 
                    ...
                    alpha = 0.2 + 0.6 * np.clip((pn_law.est_Vc - 10) / 30, 0, 1)
                    # alpha = 1
                    pn_law.omega_los_map_future = (1 - alpha) * pn_law.omega_los_map + alpha * pn_law.omega_pred

                    # 线性预测
                    pn_law.omega_pred = pn_law.predict_omega_linear(
                        omega_now = pn_law.omega_los_map,
                        t_now = rospy.Time.now().to_sec(), 
                        window_size = 10,      # 调节平滑度
                        predict_horizon = 1.0, # 预测1秒后的值
            ...
            q, thrust_cmd, roll_cmd, pitch_cmd = pn_law.vector_to_attitude(commanded_accel[0], commanded_accel[1], commanded_accel[2])
