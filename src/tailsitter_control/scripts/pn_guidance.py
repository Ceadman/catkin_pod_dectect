...
# -*- coding: utf-8 -*-
import rospy
from geometry_msgs.msg import PointStamped, TwistStamped, AccelStamped
...
class Proportional_Navigation_Law:
    ...
    def pn_guidance(self, seeker_state):
        ...
        gain = np.tanh(np.maximum(r_mag, 1e-3) / (15* r_dead)) 
        ...
        #print("[PN Guidance] Vc:",Vc)
        #print("[PN Guidance] Omega:",Omega)
    ...
    def get_commanded_accel(self, seeker_now, missile_state):
        ...
        if MEMORY_GUIDANCE and target_lost and (not self.memory_active):
            ...
            #print(f"target_lost:{target_lost}")
...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
            ...
            # 提前估计无人机状态
            #print("simulationpp",simulation_position)
            # print("self.positio3333n", pn_law.position)
            # print("simulationVEL",simulation_velocity)
            # print("simulationDT",dt)
            simulation_position = np.array([pn_law.position.x, pn_law.position.y, pn_law.position.z])
            simulation_velocity = np.array([pn_law.linear_velocity.x, pn_law.linear_velocity.y, pn_law.linear_velocity.z])
            
            simulation_position += simulation_velocity * dt
            # print("simulationp1p",simulation_position)
            # print("self.positio4444n", pn_law.position)
            ...
            if pn_law.target_in_img == False and pn_law.memory_active == False:
                ...
                #rospy.loginfo("get image successful!!!!!!!!!");
            ...
            rospy.loginfo_throttle(0.1, f"[PN Guidance] Distance: {distance:.2f}m")
            ...
            rospy.loginfo_throttle(
                0.1,
                "command: [% .2f, % .2f, % .2f]" % tuple(commanded_velocity)
            )
